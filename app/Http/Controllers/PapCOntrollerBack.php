    ...
    public function searchPaps(Request $request){
        ...
        for ($i = 0; $i < count($result); $i++) {
            ...
            /* $pdf->AddPage();
             ...
             $pdf->Ln(5);
             $pdf->SetFont('Arial','',5);
             $pdf->Cell(98,5,utf8("Je, BERD Consultant, certifie avoir validé les documents cités dasn cette liste, et que ces documents reçus & acceptés par"),'LR',0,'',0);
             $pdf->Cell(95,5,utf8(""),'R',0,'',0);
             $pdf->Ln(3);
             ...
             $pdf->Cell(95,5,utf8(""),'R',0,'',0);
             $pdf->Ln(3);
             $pdf->Cell(98,5,utf8("correctes, et que les documents contenues dasn le dossier, satisfont à toutes les exigences de l'accord PAR approuvé avec"),'LR',0,'',0);
             $pdf->Cell(95,5,utf8(""),'R',0,'',0);
             $pdf->Ln(3);
           ...
           */
            /*
             ...
             $pdf->Ln(8);
             $pdf->Cell(190, 10, utf8("DANS LE CADRE DE LA RÉHABILIATION DE LA ROUTE : {$result[$i]->NOM_AXE}"), 0, 0, 'C');
             $pdf->Ln(8);
...
localite.NUM_LOCALITE=occupant.NUM_LOCALITE AND occupant.ID_PAP='$request->ID_PAP'
    ...
    public function makePDF($result)
    {
        for ($i = 0; $i < count($result); $i++){
            ...
            $pdf->Ln(6);
            $pdf->SetFont('Arial','',11);
            $pdf->Cell(0,10,utf8("ont relevé que M/Mme {$result[$i]->NOM_OCCUP} {$result[$i]->PRENOM_OCCUP}   tel qu'il/elle a été identifié ci-dessus figure parmi les personnes"),0,0);
            $pdf->Ln(6);
            $pdf->SetFont('Arial','',11);
            $pdf->Cell(0,10,utf8("affectées par le projet."),0,0);
            $pdf->Ln(10);
            $pdf->SetFont('Arial','',11);
            ...
            $pdf->Ln(10);
            $pdf->SetFont('Arial','',11);
            $pdf->Cell(0,10,utf8("M/Mme {$result[$i]->NOM_OCCUP} {$result[$i]->PRENOM_OCCUP} reconnaît avoir été informé(e) et impliqué(e) dans le processus d'identification et "),0,0);
            $pdf->Ln(6);
            $pdf->Cell(0,10,utf8("d'évaluation des biens affectés. Il/Elle atteste par ailleurs que les négociations se sont déroulées dans un"),0,0);
            $pdf->Ln(6);
            $pdf->Cell(0,10,utf8("esprit convivial et que son consentement a été donné librement, sans influence ou contrainte aucune."),0,0);
            ...
            $pdf->Ln(10);
            $pdf->SetFont('Arial','',11);
            $pdf->Cell(0,10,utf8("M/Mme {$result[$i]->NOM_OCCUP} {$result[$i]->PRENOM_OCCUP} atteste que les biens énumérés dans la fiche individuelle de compensation financière"),0,0);
            $pdf->Ln(6);
            $pdf->Cell(0,10,utf8("(en annexe et faisant partie intégrante du présent protocole), sont exhaustifs et donc que l'ensemble de"),0,0);
            $pdf->Ln(6);
            $pdf->Cell(0,10,utf8("ses biens affectés on été pris en compte dasn le cadre de la présente procédure."),0,0);
            ...
            $pdf->Ln(10);
            $pdf->Cell(130,5,utf8("1-) Transfert bancaire (le RIB est joint à mon dossier)"),0,0,'',0);
